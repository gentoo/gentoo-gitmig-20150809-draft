...
# $Header: /var/cvsroot/gentoo-x86/gnome-base/gdm/gdm-2.4.1.7-r1.ebuild,v 1.7 2004/02/10 06:14:59 darkspecter Exp $
...
src_compile() {
	...
	if [ `use X` ]; then
		...
		use xinerama \
			...
			|| myconf="${myconf} --with-xinerama=no"
	else
		myconf="${myconf} --with-xdmcp=no --with-xinerama=no"
	fi

	use tcpd \
		&& myconf="${myconf} --with-tcp-wrappers=yes" \
		|| myconf="${myconf} --with-tcp-wrappers=no"
