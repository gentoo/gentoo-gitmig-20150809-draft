...
# $Header: /var/cvsroot/gentoo-x86/dev-db/postgresql/postgresql-7.4.1-r2.ebuild,v 1.1 2004/01/16 16:45:55 nakano Exp $
...
src_install() {
	if use perl; then
		...
		sed -e "s:(INST_DYNAMIC) /usr/lib:(INST_DYNAMIC) ${D}/usr/lib/postgresql:" \
	...
	make DESTDIR=${D} LIBDIR=${D}/usr/lib/postgresql install || die
	...
	insinto /etc/env.d/
	newins ${FILESDIR}/postgresql.env-${PV} 50postgresql || die
}
