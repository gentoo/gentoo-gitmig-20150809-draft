...
# $Header: /var/cvsroot/gentoo-x86/dev-db/postgresql/postgresql-7.4.5-r1.ebuild,v 1.6 2005/02/24 20:59:32 kaiowas Exp $
...
IUSE="ssl nls java python tcltk perl libg++ pam readline zlib doc pg-hier pg-vacuumdelay pg-intdatetime selinux"
...
src_unpack() {
	unpack ${A} || die
	epatch ${FILESDIR}/${P}-${PR}-gentoo.patch
	if use pg-hier; then
		cd ${S} || die
