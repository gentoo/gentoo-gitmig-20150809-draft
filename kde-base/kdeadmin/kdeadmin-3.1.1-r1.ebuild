...
# $Header: /var/cvsroot/gentoo-x86/kde-base/kdeadmin/kdeadmin-3.1.1-r1.ebuild,v 1.2 2003/04/25 15:53:26 danarmak Exp $
inherit kde-dist 
...
# we generally want to compile the lilo config module, if only to ensure consistent behaviour
# of the package across both lilo and grub systems, because configure when left to its own
# devices will build lilo-config or not basd on whether lilo is present in the path.
# so, we make configure always build it by removing the configure.in.in file that checks for
# lilo's presense
src_unpack() {
