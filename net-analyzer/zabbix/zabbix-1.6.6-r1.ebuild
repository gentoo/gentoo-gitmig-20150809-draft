...
# $Header: /var/cvsroot/gentoo-x86/net-analyzer/zabbix/zabbix-1.6.6-r1.ebuild,v 1.1 2009/10/06 16:24:34 patrick Exp $
...
src_install() {
	...
	if useq server; then
		...
		doins \
			...
			"${FILESDIR}/${PV}"/zabbix_trapper.conf
		doinitd \
	...
	if useq proxy; then
		doinitd \
			"${FILESDIR}/${PV}"/init.d/zabbix-proxy
		dosbin \
			src/zabbix_proxy/zabbix_proxy
		insinto /etc/zabbix
		doins \
			"${FILESDIR}/${PV}"/zabbix_proxy.conf
		dodir \
			/usr/share/zabbix/database
		insinto /usr/share/zabbix/database
		doins -r \
