...
# $Header: /var/cvsroot/gentoo-x86/net-www/mozilla/mozilla-1.3_beta.ebuild,v 1.2 2003/03/11 21:11:46 seemant Exp $
...
src_unpack() {
	...
	# Get nsProfileDirServiceProvider to send profile shutdown notifications.
	# This fix Galeon, etc not saving cookies on exit, bug #15821, and mozilla
	# bugs:
	#
	#   http://bugzilla.mozilla.org/show_bug.cgi?id=188214
	#   http://bugzilla.mozilla.org/show_bug.cgi?id=192555
	#
	# <azarah@gentoo.org> (22 Feb 2003)
	cd ${S}; epatch ${FILESDIR}/1.3/${PN}-1.3-provider-shutdown.patch
...
src_compile() {
	...
	then
		...
		myconf="${myconf} --disable-xft --enable-freetype2"
	fi
