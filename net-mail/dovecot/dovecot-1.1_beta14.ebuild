...
# $Header: /var/cvsroot/gentoo-x86/net-mail/dovecot/dovecot-1.1_beta14.ebuild,v 1.1 2008/01/22 16:17:23 wschlich Exp $

inherit autotools eutils ssl-cert

MY_P="${P/_/.}"
S="${WORKDIR}/${MY_P}"
SIEVE="dovecot-sieve-1.1.2"
SIEVE_S="${WORKDIR}/${SIEVE}"
#MANAGESIEVE="dovecot-1.0.7-MANAGESIEVE-v9"

SRC_URI="http://dovecot.org/releases/1.1/beta/${MY_P}.tar.gz
sieve? ( http://dovecot.org/releases/sieve/${SIEVE}.tar.gz )"
#managesieve? ( http://sinas.rename-it.nl/~sirius/${MANAGESIEVE}.diff.gz )"
...
src_unpack() {
	unpack ${A}
	cd "${S}"
#	use managesieve && epatch "${DISTDIR}"/${MANAGESIEVE}.diff.gz
}
