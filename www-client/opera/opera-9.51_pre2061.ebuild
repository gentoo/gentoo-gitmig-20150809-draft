...
# $Header: /var/cvsroot/gentoo-x86/www-client/opera/opera-9.51_pre2061.ebuild,v 1.1 2008/07/01 16:23:24 jer Exp $
...
SRC_URI="
	amd64? ( ia32? ( ${O_URI}intel-linux/${O_VER}-${O_SUFF}.gcc4-static-qt3.i386.tar.bz2 )
		     !ia32? ( ${O_URI}x86_64-linux/${O_VER}-${O_SUFF}.gcc4-shared-qt3.x86_64.tar.bz2 )
			)
...
RDEPEND="media-libs/libexif
	...
	amd64? ( ia32? ( app-emulation/emul-linux-x86-qtlibs )
			 !ia32? ( =x11-libs/qt-3* )
	)
	ppc? ( =x11-libs/qt-3* )
...
pkg_setup() {
	if use amd64; then
		if use ia32; then
			...
			S="${WORKDIR}/${O_VER}-${O_SUFF}.gcc4-shared-qt3.x86_64"
		fi
	fi
