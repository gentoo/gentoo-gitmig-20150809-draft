...
# $Header: /var/cvsroot/gentoo-x86/www-client/opera/opera-9.60_pre2424.ebuild,v 1.1 2008/09/10 12:59:50 jer Exp $
...
MY_LINGUAS="cs de en en-GB es-ES es-LA fi fr hu it ja nb nl nn pl pt ru sv tr uk zh-CN"
...
SRC_URI="
	ppc? ( ${O_URI}linux${O_FTP}ppc/shared/${O_VER}.gcc4-shared-qt3.ppc.tar.bz2 )
	x86-fbsd? (
		qt-static? ( ${O_URI}unix/freebsd/960b1/en/intel/static/${O_VER}-freebsd5-static-qt3.i386.tar.bz2 )
		!qt-static? ( ${O_URI}unix/freebsd/960b1/en/intel/shared/${O_VER}-freebsd5-shared-qt3.i386.tar.bz2 )
	)
	qt-static? (
		qt3-static? (
			amd64? (
				ia32? ( ${O_URI}linux${O_FTP}i386/static/${O_VER}.gcc4-static-qt3.i386.tar.bz2 )
				!ia32? ( ${O_URI}linux${O_FTP}x86_64/shared/${O_VER}.gcc4-shared-qt3.x86_64.tar.bz2 )
			)
			x86? ( ${O_URI}linux${O_FTP}i386/static/${O_VER}.gcc4-static-qt3.i386.tar.bz2 )
		)
		!qt3-static? (
			amd64? (
				ia32? ( ${O_URI}linux${O_FTP}i386/${O_VER}.gcc4-qt4.i386.tar.bz2 )
				!ia32? ( ${O_URI}linux${O_FTP}x86_64/shared/${O_VER}.gcc4-shared-qt3.x86_64.tar.bz2 )
			)
			x86? ( ${O_URI}linux${O_FTP}i386/${O_VER}.gcc4-qt4.i386.tar.bz2 )
		)
	)
	!qt-static? (
		qt3-static? (
			amd64? (
				ia32? ( ${O_URI}linux${O_FTP}i386/shared/${O_VER}.gcc4-shared-qt3.i386.tar.bz2 )
				!ia32? ( ${O_URI}linux${O_FTP}x86_64/shared/${O_VER}.gcc4-shared-qt3.x86_64.tar.bz2 )
			)
			x86? ( ${O_URI}linux${O_FTP}i386/shared/${O_VER}.gcc4-shared-qt3.i386.tar.bz2 )
		)
		!qt3-static? (
			amd64? (
				ia32? ( ${O_URI}linux${O_FTP}i386/shared/${O_VER}.gcc4-shared-qt3.i386.tar.bz2 )
...
opera_cd() {
	cd "${WORKDIR}"/${O_VER}* || die "failed to enter work directory"
	S="$(pwd)"
	einfo "Working in ${S}"
}
