...
# $Header: /var/cvsroot/gentoo-x86/www-client/opera/opera-9.51_pre2050.ebuild,v 1.1 2008/06/23 00:48:35 jer Exp $
...
# +qt-static -qt3-static = qt4
# -qt-static -qt3-static = shared-qt3
# -qt-static +qt3-static = static-qt3

SRC_URI="
	ppc? ( ${O_URI}ppc-linux/${O_VER}-${O_SUFF}.gcc4-shared-qt3.ppc.tar.bz2 )
	x86-fbsd? ( ${O_URI}intel-freebsd/${O_VER}-${O_SUFF}.freebsd5-shared-qt3.i386.tar.bz2 )
	qt-static? (
		qt3-static? (
			x86? ( ${O_URI}intel-linux/${O_VER}-${O_SUFF}.gcc4-static-qt3.i386.tar.bz2 )
		)
		!qt3-static? (
			x86? ( ${O_URI}intel-linux/${O_VER}-${O_SUFF}.gcc4-qt4.i386.tar.bz2 )
		)
	)
	!qt-static? (
		qt3-static? (
		...
		!qt3-static? (
			x86? ( ${O_URI}intel-linux/${O_VER}-${O_SUFF}.gcc4-shared-qt3.i386.tar.bz2 )
		)
	)
	"
...
pkg_setup() {
	use ppc && S="${WORKDIR}/${O_VER}-${O_SUFF}.gcc4-shared-qt3.ppc"
	use x86-fbsd && S="${WORKDIR}/${O_VER}-${O_SUFF}-freebsd5-shared-qt3.i386"
	if use x86; then
		if use qt-static; then
			if use qt3-static; then
				S="${WORKDIR}/${O_VER}-${O_SUFF}.gcc4-static-qt3.i386"
			else
				S="${WORKDIR}/${O_VER}-${O_SUFF}.gcc4-qt4.i386"
			fi
		else
			if use qt3-static; then
				S="${WORKDIR}/${O_VER}-${O_SUFF}.gcc4-static-qt3.i386"
			else
				S="${WORKDIR}/${O_VER}-${O_SUFF}.gcc4-shared-qt3.i386"
			fi
		fi
	fi
}
