...
# $Header: /var/cvsroot/gentoo-x86/sys-apps/portage/portage-2.1_pre10-r5.ebuild,v 1.1 2006/05/08 20:45:02 zmedico Exp $
...
[ -n "${PATCHVER}" ] && \
	SRC_URI="${SRC_URI} mirror://gentoo/${PN}-patches-${PV}${PATCHVER}.tar.bz2"
unset PATCHVER
...
src_unpack() {
	...
	if [ -d "${WORKDIR}/patch" ]; then
		local p
		for p in $(ls "${WORKDIR}"/patch | sort); do
			epatch "${WORKDIR}"/patch/${p}
		done
	fi

	epatch "${FILESDIR}"/${PV%_*}/${PV/*_}/1150_r3334_make_profile_realpath.patch
