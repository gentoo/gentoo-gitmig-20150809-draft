...
# $Header: /var/cvsroot/gentoo-x86/sys-apps/portage/portage-2.0.54-r1.ebuild,v 1.1 2006/04/18 11:13:58 solar Exp $
...
RDEPEND="!build? ( >=sys-apps/sed-4.0.5 dev-python/python-fchksum >=dev-lang/python-2.3 userland_GNU? ( sys-apps/debianutils ) >=app-shells/bash-2.05a ) !userland_Darwin? ( >=app-misc/pax-utils-0.1.10 sys-apps/sandbox ) selinux? ( >=dev-python/python-selinux-2.15 ) doc? ( app-portage/portage-manpages )"
...
src_unpack() {
	...
	local patches=$(echo bin_{ebuild.sh,emerge,fowners,fperms,prepallstrip,prepstrip} man_{ebuild.5,portage.5})
	for x in ${patches}; do
		epatch "${FILESDIR}"/${PV}/${x}.patch
	done
}
...
src_compile() {
	...
	cd "${S}"/src
	$(tc-getCC) ${CFLAGS} -o tbz2tool tbz2tool.c ${LDFLAGS}
