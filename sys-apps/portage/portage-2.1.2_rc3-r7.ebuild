...
# $Header: /var/cvsroot/gentoo-x86/sys-apps/portage/portage-2.1.2_rc3-r7.ebuild,v 1.1 2006/12/19 04:13:24 zmedico Exp $
...
src_test() {
	local test_runner="./tests/runTests"
	chmod +x ${test_runner}
	PYTHONPATH="${S}:${S}/pym:${PYTHONPATH}" ${test_runner} || \
		die "test(s) failed"
}
...
pkg_postinst() {
	...
	echo
	ewarn "In portage-2.1.2_rc2, the implicit RDEPEND behavior has been reverted back to"
