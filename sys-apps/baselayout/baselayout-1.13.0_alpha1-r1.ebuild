...
src_compile() {
	...
	local PAMFLAGS=
	use kernel_linux && use pam && PAMFLAGS="-DHAVE_PAM -lpam"
...
src_install() {
	...
	[[ -e "${D}"/etc/shadow ]] && fperms 0600 /etc/shadow
	for x in passwd shadow group fstab hosts ; do
		[[ -e "${D}/etc/${x}" ]] || continue
		mv "${D}/etc/${x}" "${D}"/usr/share/baselayout
	done
...
pkg_postinst() {
	# Copy our files in share back if they do not exist
	for x in passwd shadow group fstab hosts ; do
