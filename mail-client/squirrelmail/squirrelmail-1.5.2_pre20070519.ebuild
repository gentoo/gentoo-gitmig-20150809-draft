...
# $Header: /var/cvsroot/gentoo-x86/mail-client/squirrelmail/squirrelmail-1.5.2_pre20070519.ebuild,v 1.1 2007/05/19 03:13:01 eradicator Exp $
...
LOCALES_VER=1.5.1-20060409

#MY_P=${P/_rc/-RC}
MY_P="squirrelmail.devel"
S="${WORKDIR}/${MY_P}"

PLUGINS_LOC="http://www.squirrelmail.org/plugins"

#mirror://sourceforge/${PN}/${MY_P}.tar.bz2
SRC_URI="mirrors://gentoo/squirrelmail-20070519_0000-SVN.devel.tar.bz2
...
src_unpack() {
#	unpack ${MY_P}.tar.bz2
	unpack squirrelmail-20070519_0000-SVN.devel.tar.bz2

	cd ${S}

#	mv config/config_default.php config/config.php
	mv config/config_local.example.php config/config_local.php

	sed -i "s:'/var/local/squirrelmail/data':SM_PATH . 'data/':" config/config_default.php
...
src_install() {
	...
	for file in config/config_local.php plugins/*/{config.php,sqspell_config.php,gpg_local_prefs.txt}; do
		if [[ -f ${file} ]] ; then
