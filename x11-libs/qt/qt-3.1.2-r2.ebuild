...
_EOF_
        ...
        # use our CFLAGS
	cp qmake.conf qmake.conf.no-cxxflags
        sed -e "s:= -O2:= ${CXXFLAGS}:" qmake.conf.orig > qmake.conf
...
src_install() {
	...
	# we don't want all qt apps to use the CXxFLAGS we had here
	cd ${D}/${QTBASE}/mkspecs/linux-g++
	mv -f qmake.conf.no-cxxflags qmake.conf
