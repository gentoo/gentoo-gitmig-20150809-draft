...
# $Header: /var/cvsroot/gentoo-x86/dev-lang/python/python-2.1.1-r2.ebuild,v 1.1 2001/11/16 05:46:07 chouser Exp $
...
src_compile() {
	...
	# also update $mylibs which will be used to create python-config
	cd ${S}
	scmd=""
	mylibs=""
	...
	if use readline; then
		...
		mylibs="$mylibs -lreadline -lncurses"
	fi
	...
	if use tcltk; then
		...
		scmd="$scmd  s:#\(\t-lX11.*\):\1:;"
		mylibs="$mylibs -ltk$tkv -ltcl$tclv -L/usr/X11R6/lib -lX11"
	fi
	...
	mylibs="-lm -lpython${PV} $mylibs -lz -lutil"
	# now apply the Setup adjustments and create python-config
	sed "$scmd" Modules/Setup.dist > Modules/Setup
	echo -e "#!/bin/sh\necho $mylibs" > python-config
...
src_install() {
	...
	if use tcltk; then
		...
		mv "${S}/Tools/idle" "${D}/usr/lib/python2.0/tools/"
