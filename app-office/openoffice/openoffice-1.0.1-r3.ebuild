...
# $Header: /var/cvsroot/gentoo-x86/app-office/openoffice/openoffice-1.0.1-r3.ebuild,v 1.1 2002/12/11 22:42:09 azarah Exp $
...
# This is never going to work, as the dir do not exist ....
# Anyhow, the correct version of the lib is handled later on by the
# 'gcc-libstdcxx-major-version' function ... (Az)
...
#	perl -pi -e "s| -I$(gcc-libpath)/include||g" ${LinuxEnvSet}
	...
	if [ "${NEW_GCC}" -eq "1" ]
	then
		local gcc_path="$(/usr/sbin/gcc-config --get-bin-path)"
