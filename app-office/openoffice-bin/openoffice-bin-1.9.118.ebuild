...
# $Header: /var/cvsroot/gentoo-x86/app-office/openoffice-bin/openoffice-bin-1.9.118.ebuild,v 1.1 2005/07/21 15:42:28 suka Exp $
...
SRC_URI="mirror://openoffice/developer/680_m${SNV}/OOo_${PV}_LinuxIntel_rpm_install.tar.gz
...
src_unpack() {
	unpack OOo_${PV}_LinuxIntel_rpm_install.tar.gz
	...
	for i in ${LINGUAS}; do
		...
		if [ ${i} != "en" ] ; then
			if [ -e ${DISTDIR}/${LANGPACK}_${i}.sh ] ; then
				tail -n +155 ${DISTDIR}/${LANGPACK}_${i}.sh > ${S}/${LANGPACK}_${i}.tar.gz || die
