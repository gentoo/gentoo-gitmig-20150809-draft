...
# $Header: /var/cvsroot/gentoo-x86/app-cdr/cdw/cdw-0.2.0_pre3.ebuild,v 1.1 2003/11/03 07:30:05 mr_bones_ Exp $
...
src_compile() {
	...
	use oggvorbis \
		&& myconf="${myconf} --with-oggenc=/usr/include/vorbis/" \
		|| myconf="${myconf} `use_with oggvorbis oggenc`"

	use encode \
		&& myconf="${myconf} --with-lame=/usr/include/lame/" \
