...
#X_PATCHES="mirror://gentoo/XFree86-4.3.0-patches-${PATCH_VER}.tar.bz2"
...
GENTOO_FILES="${P}-gentoo-files-${FILES_VER}.tar.bz2"
...
src_unpack() {
	...
	# cyfred::
	# Apply patch to create xc/exports/lib
	# Apply patch to redirect libXau.a locations
	cd ${S}
	epatch ${FILES_DIR}/exports-lib.patch
	cd ${S}/programs/Xserver/
	epatch ${FILES_DIR}/AuthLib.patch

	cd ${S}
	# cyfred: im still going through the entire patchset and
	# am under the impression that somethings still arent good so
	# im only applying the acecad patch for the time being to make
	# sure the build is successful.
	# UPDATE:: These patches from 4.3.0 cause problems all over
	# am removing totally for just the one for now.
	epatch ${FILES_DIR}/acecad.patch
