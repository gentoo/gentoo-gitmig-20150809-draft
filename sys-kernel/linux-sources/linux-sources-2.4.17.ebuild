...
patchorama() {
	...
	do
		...
		then
			x=${x%%.bz2}
			bzip2 -dc ${x}.bz2 > ${patchdir}/${x##*/} || die
		elif [ "${x##*.}" = "gz" ]
  		then
			x=${x%%.gz}
			gzip -dc ${x}.gz > ${patchdir}/${x##*/} || die
		else
			cp ${x} ${patchdir}
		fi
		x=${patchdir}/${x##*/}
		if [ -f ${FILESDIR}/${PVR}/${x##*/} ]
		then
			echo ">>> Fixing ${x##*/} for use with Gentoo..."
			patch < ${FILESDIR}/${PVR}/${x##*/} || {
				echo
				echo "!!!Failed to fix ${x##*/}!!!"
				echo
				die
			}
		fi
		...
		patch -p1 -l < ${x} || {
			echo
			echo "!!!Failed to apply ${x##*/}!!!"
			echo "Check that you are not missing patches in:"
			echo
			echo "    ${FILESDIR}/${PVR}/"
			echo
			die
