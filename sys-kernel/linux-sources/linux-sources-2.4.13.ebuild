...
# $Header: /var/cvsroot/gentoo-x86/sys-kernel/linux-sources/linux-sources-2.4.13.ebuild,v 1.1 2001/11/04 17:04:53 azarah Exp $
...
http://www.tech9.net/rml/linux/preempt-kernel-rml-${PREEV}.patch
http://www.uow.edu.au/~andrewm/linux/${KV}-low-latency.patch.gz
...
src_unpack() {
	...
	cd ${S}
	patch -p1 < ${DISTDIR}/preempt-kernel-rml-${PREEV}.patch
	cat ${DISTDIR}/${EXT3P}.gz | gzip -dc | patch -p1 || die
	...
	cd ${S}
	cat ${DISTDIR}/${KV}-low-latency.patch.gz | gzip -d | patch -p1 || die

	#apply ACPI updates
	cat ${DISTDIR}/acpi-${ACPIV}.diff.gz | gzip -d | patch -p1
	#|| die
	#we removed the || die because it fails with one hunk.  this is not
	#critical (form changes), so accept if for now until we have a
	#patch for 2.4.13
