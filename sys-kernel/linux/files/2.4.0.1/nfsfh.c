 ...
 */
static struct dentry *nfsd_iget(struct super_block *sb, struct i_label * ilabelp, __u32 generation)
{
	...
	inode = iget4(sb, ilabelp, NULL, NULL);
...
find_fh_dentry(struct super_block *sb, struct i_label * ilabelp, int generation,
	       struct i_label * ilabelp_dir, int needpath)
{
 ...
 retry:
	...
	result = nfsd_iget(sb, ilabelp, generation);
	if (IS_ERR(result)
	    || !(result->d_flags & DCACHE_NFSD_DISCONNECTED)
	...
	if (!S_ISDIR(result->d_inode->i_mode)) {
		...
		if (ilabelp_dir == 0)
			...
			dentry = nfsd_iget(sb, ilabelp_dir, 0);
			err = PTR_ERR(dentry);
			if (IS_ERR(dentry))
				goto err_result;
			err = -ESTALE;
...
fh_verify(struct svc_rqst *rqstp, struct svc_fh *fhp, int type, int access)
{
	...
	if (!fhp->fh_dentry) {
		...
		if (fh->fh_version == 1) {
			...
			sbp = exp -> ex_dentry -> d_inode -> i_sb;
			if (sbp -> s_nfsd_op && sbp -> s_nfsd_op -> fh_to_ilabel) {
				struct i_label ilabel, ilabel_dir;
				int result;
				struct fh2ilbl_args args = {
					& ilabel, & ilabel_dir, 0
				};

				result = sbp -> s_nfsd_op ->
					fh_to_ilabel (sbp, fh -> fh_fileid_type, datap, data_left, & args);
				if (result != 0) {
					goto out;
				}

				dentry = find_fh_dentry(sbp, args.ilabelp, args.i_generation,
							args.ilabelp_dir,
			...
			case 1:
				...
				dentry = find_fh_dentry(exp->ex_dentry->d_inode->i_sb,
							(struct i_label *) (datap + 0), datap[1],
							0,
							!(exp->ex_flags & NFSEXP_NOSUBTREECHECK));
				break;
			case 2:
			...
			dentry = find_fh_dentry(exp->ex_dentry->d_inode->i_sb,
						(struct i_label *) & (fh->ofh_ino),
						fh->ofh_generation,
						(struct i_label *) & (fh->ofh_dirino),
						!(exp->ex_flags & NFSEXP_NOSUBTREECHECK));
		}
l_chkdentry:
		if (IS_ERR(dentry)) {
		      ...
		      __u32 **datapp, int maxsize)
{
	...
	if (sbp -> s_nfsd_op && sbp -> s_nfsd_op -> encode_fh) {
		return (sbp -> s_nfsd_op -> encode_fh (dentry, exp->ex_flags, datapp, maxsize));
	}
