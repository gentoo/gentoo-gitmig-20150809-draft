...
#	     ftp://ftp.reiserfs.com/pub/reiserfs-for-2.4/linux-${OKV}-reiserfs-${RV}.patch.gz
    ...
    if [ "`use xfs`" ]
    then
	SRC_URI="${SRC_URI} ftp://ftp.ibiblio.org/pub/Linux/distributions/gentoo/gentoo-sources/linux-${KV}-xfs-${XFSV}-gentoo.diff.gz"
    fi
fi
...
src_unpack() {
	...
	then
	    ...
	    try gzip -dc ${DISTDIR}/linux-${KV}-xfs-${XFSV}-gentoo.diff.gz | patch -p1
	fi
	...
	then
	    ...
	    if [ "`use xfs`" ]
	    then
		try patch -l -p1 < ${FILESDIR}/${PVR}/linux-${KV}-xfs-${XFSV}-pre-lvm-gentoo.diff
	    fi
	...
	then
	    ...
	    try patch -p1 < ${FILESDIR}/${PVR}/linux-${KV}-xfs-${XFSV}-cleanup-gentoo.diff
	fi
