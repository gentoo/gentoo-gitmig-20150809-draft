...
src_unpack() {
	...
	then
		...
		patch -p0 < ${FILESDIR}/${KV}/nfsfh-ac-fix.diff || die
		echo "Applying reiserfs-knfsd patch..."
		gzip -dc ${DISTDIR}/linux-${OKV}-knfsd-${KNV}.patch.gz | patch -p1 || die
		echo "Applying ac-nfsfh-knfsd patch..."
		patch -p0 < ${FILESDIR}/${KV}/nfsfh-ac-knfsd.diff || die
		echo "Applying reiserfs-procinfo patch..."
		gzip -dc ${DISTDIR}/linux-${OKV}-procinfo-${PIV}.patch.gz | patch -p1 || die
