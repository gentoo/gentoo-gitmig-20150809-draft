...
# $Header: /var/cvsroot/gentoo-x86/sys-kernel/xen-sources/xen-sources-2.6.16-r1.ebuild,v 1.1 2006/04/10 12:02:56 chrb Exp $
...
SRC_URI="${KERNEL_URI} http://www.cl.cam.ac.uk/Research/SRG/netos/xen/downloads/xen-${XEN_VERSION}-src.tgz"
...
src_unpack() {
	...
	make LINUX_SRC_PATH=${DISTDIR} -f buildconfigs/mk.linux-2.6-xen \
		linux-${PV}-xen/include/linux/autoconf.h
	t=linux-${PV}-xen
	mv ${t} ${WORKDIR}/linux-${KV}
	rm -rf ${WORKDIR}/linux-${PV}
	rm -rf ${WORKDIR}/${MY_P}
}
