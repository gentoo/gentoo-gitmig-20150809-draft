...
# $Header: /var/cvsroot/gentoo-x86/media-video/mtxdrivers-pro/mtxdrivers-pro-1.0_beta4.ebuild,v 1.1 2003/06/06 15:29:40 prez Exp $                     
...
src_unpack() {
    if [ ! -f "${DISTDIR}/${At}" ]; then
		...
		einfo "the Matrox Parhelia drivers with GL support."
		die
    fi
	unpack ${At}
}
...
src_install() {
	...
	dodir /usr/lib /usr/include/GL /lib/modules/${Kversion}/kernel/drivers/video
    dodir ${Xpath}/lib/modules/drivers ${Xpath}/lib/modules/extensions
	...
	# OpenGL
	install -m 755 ${S}/xfree86/${Xversion}/libglx.a ${D}/${Xpath}/lib/modules/extensions
	install -m 644 ${S}/include/GL/gl.h ${D}/usr/include/GL
	install -m 644 ${S}/include/GL/glx.h ${D}/usr/include/GL
	install -m 644 ${FILESDIR}/glext.h ${D}/usr/include/GL
	dolib ${S}/lib/libGL.so.${GLversion}
}
