...
# $Header: /var/cvsroot/gentoo-x86/sys-fs/udev/udev-117.ebuild,v 1.2 2008/06/15 05:54:32 zmedico Exp $
...
src_unpack() {
	...
	MD5=${MD5/  -/}
	if [[ ${MD5} != 42ee128b936ebad2e80f011597c869b2 ]]
	then
		echo
...
src_install() {
	...
	newexe "${FILESDIR}"/net-104-r10.sh net.sh	|| die "net.sh not installed properly"
	...
	cat "${FILESDIR}"/udev.conf.115-r6 >> "${D}"/etc/udev/udev.conf
	...
	newins "${FILESDIR}"/udev-start-115-r6.sh udev-start.sh
