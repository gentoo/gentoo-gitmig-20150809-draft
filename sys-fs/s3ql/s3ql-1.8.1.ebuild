...
# $Header: /var/cvsroot/gentoo-x86/sys-fs/s3ql/s3ql-1.8.1.ebuild,v 1.1 2011/12/08 10:04:41 radhermit Exp $
...
src_test() {
	if [[ ${EUID} -ne 0 ]] ; then
		ewarn "tests require root privileges so userpriv must be disabled"
	else
		addwrite /dev/fuse
		distutils_src_test
	fi
