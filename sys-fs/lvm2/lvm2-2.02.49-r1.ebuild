...
# $Header: /var/cvsroot/gentoo-x86/sys-fs/lvm2/lvm2-2.02.49-r1.ebuild,v 1.2 2009/07/31 17:51:02 ssuominen Exp $
...
src_prepare() {
	...
	epatch "${FILESDIR}"/${PN}-2.02.49-lockdir-override.patch
	eautoreconf
}
...
src_install() {
	...
	newins "${FILESDIR}"/lvm2-start.sh-2.02.49-r1 lvm-start.sh || die
	newins "${FILESDIR}"/lvm2-stop.sh-2.02.49-r1 lvm-stop.sh || die
