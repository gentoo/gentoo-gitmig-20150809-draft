...
# $Header: /var/cvsroot/gentoo-x86/games-fps/warsow/warsow-0.3.1-r1.ebuild,v 1.1 2007/07/05 15:57:37 nyhm Exp $
...
src_install() {
	...
	dodir "${GAMES_DATADIR}"/${PN}/BattlEye
	exeinto "$(games_get_libdir)"/${PN}

	if use x86 ; then
		cd "${WORKDIR}"/${PN}
		for so in BattlEye/*_x86.so ; do
			doexe ${so} || die "doexe ${so} failed"
			dosym "$(games_get_libdir)"/${PN}/${so##*/} \
				"${GAMES_DATADIR}"/${PN}/${so} \
				|| die "dosym ${so} failed"
		done
		...
		cd "${DISTDIR}"
		for so in *_x64.so ; do
			doexe ${so} || die "doexe ${so} failed"
			dosym "$(games_get_libdir)"/${PN}/${so} \
				"${GAMES_DATADIR}"/${PN}/BattlEye/${so} \
				|| die "dosym ${so} failed"
		done
	fi
